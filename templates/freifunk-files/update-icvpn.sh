...
sudo -u nobody /opt/freifunk/icvpn-scripts/mkbgp -4 -f bird -d peers -p icvpn_ -s "$DATADIR" -x bodensee > /etc/bird/bird.d/icvpn.conf
sudo -u nobody /opt/freifunk/icvpn-scripts/mkbgp -6 -f bird -d peers -p icvpn_ -s "$DATADIR" -x bodensee > /etc/bird/bird6.d/icvpn.conf
