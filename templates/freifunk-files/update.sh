...
[ -n "$ipv6_address" ] || { echo "(E) ipv6_address not set!"; exit 1; }
...
if ip -6 addr add "$ipv6_address/64" dev bat0 2> /dev/null; then
	echo "(I) Set IP-Address of bat0 to $ipv6_address"
fi
...
{
	echo -n "{\"link\" : \"http://[$ipv6_address]/index.html\", \"label\" : \"Freifunk Gateway $name\"}"
